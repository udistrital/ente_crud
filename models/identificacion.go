...
func GetAllIdentificacion(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Identificacion)).RelatedSel(3)
	// query k=v
	for k, v := range query {
...
func UpdateIdentificacionById(m *Identificacion) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Update(m, "Ente", "TipoIdentificacion", "NumeroIdentificacion", "FechaExpedicion", "LugarExpedicion", "Soporte", "FechaModificacion"); err == nil {
